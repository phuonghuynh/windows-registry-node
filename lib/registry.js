...
var api = {
    ...
    queryValueForKeyObject: function (key, valueName) {
        ...
        var valueNameUtf16 = Buffer.from(valueName, 'utf16le');
        var result = advApi.RegQueryValueExW(key.handle.deref(), valueNameUtf16, null, pKeyType, null, pKeyDataLength);
        // READ VALUE
        ...
        // READ VALUE
        result = advApi.RegQueryValueExW(key.handle.deref(), valueNameUtf16, null, pKeyType, value, pKeyDataLength);
        ...
        if (value.type === types.LPTSR) {
            ...
            value = value.toString('utf16le').replace('\u0000', '');
        }
